package fiddlecomputers.mods.oreplanets.client.renderer.sky;
...
public abstract class SkyProviderBaseOP extends IRenderHandler
{
    ...
    private void renderStars()
    {
        ...
        buffer.begin(7, ConfigManagerOP.oreplanets_general.useColoredStar || ConfigManagerOP.oreplanets_general.useFancyStar ? DefaultVertexFormats.POSITION_COLOR : DefaultVertexFormats.POSITION);
        ...
        for (int i = 0; i < this.getStarCount(); ++i)
        {
            ...
            if (d4 < 1.0D && d4 > 0.01D)
            {
                ...
                for (int j = 0; j < 4; ++j)
                {
                    ...
                    if (ConfigManagerOP.oreplanets_general.useColoredStar)
                    {
                        try
                        {
                            Color color = Color.decode(SkyProviderBaseOP.starColorHexTable[rand.nextInt(SkyProviderBaseOP.starColorHexTable.length)]);
                    ...
                    {
                        if (!ConfigManagerOP.oreplanets_general.useFancyStar)
                        {
                            buffer.pos(d5 + d25, d6 + d23, d7 + d26).endVertex();
                        }
                    }
                    if (!ConfigManagerOP.oreplanets_general.useColoredStar && ConfigManagerOP.oreplanets_general.useFancyStar)
                    {
                        buffer.pos(d5 + d25, d6 + d23, d7 + d26).color(255, 255, 255, rand.nextInt(255)).endVertex();
                    }
                }
    ...
    protected void renderSolarAura(float smallAuraScale, float largeAuraScale, float starBrightness, Color rgb, float partialTicks)
    {
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder buffer = tessellator.getBuffer();
